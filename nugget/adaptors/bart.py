...
class NuggetBartAttention(BartAttention):
    # Adapted from Huggingface/Transformers v4.35.0
    ...
    def forward(
        ...
        if self.nugget_score_feeder is not None and self.nugget_score_feeder.scores is not None:
            ...
            if bsz != scores.shape[0]:
                # beam search
                scores = scores.unsqueeze(1).expand(-1, bsz // scores.shape[0], -1).flatten(0, 1)
